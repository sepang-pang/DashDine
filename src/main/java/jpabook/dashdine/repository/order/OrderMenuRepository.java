...
public interface OrderMenuRepository extends JpaRepository<OrderMenu, Long> {
    ...
    List<OrderMenu> findOrderMenusByOrderIdIn(@Param("orderIds")List<Long> orderIds);

    @Query("select om from OrderMenu om " +
            "left join fetch om.menu " +
            "where om.order.id = :orderId")
    List<OrderMenu> findOrderMenusByOrderId(@Param("orderId")Long orderId);
    ...
    List<OrderMenu> findOrderMenusByRestaurantIdIn(@Param("restaurantIds") List<Long> restaurantIds);
}
