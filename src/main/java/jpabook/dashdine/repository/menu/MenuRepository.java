...
public interface MenuRepository extends JpaRepository<Menu, Long>, MenuRepositoryCustom {
    ...
    @Query("select new jpabook.dashdine.dto.response.menu.MenuDetailsForm(m.id, m.name, m.price, m.content, m.image, m.stackQuantity, m.restaurant.id) from Menu m where m.restaurant.id = :restaurantId and m.isDeleted = false")
    ...
    @Query("select new jpabook.dashdine.dto.response.menu.MenuDetailsForm(m.id, m.name, m.price, m.content, m.image, m.stackQuantity, m.restaurant.id) from Menu m where m.id = :menuId and m.isDeleted = false")
